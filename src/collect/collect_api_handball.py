...
from config.league_ids import HANDBALL_LEAGUES

OUTPUT_DIR = Path("data/raw/api/handball")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

CONFIG = get_sport_config("handball")
API_KEY = get_api_key("handball")
BASE_URL = get_api_url("handball")
...
for league in LEAGUES:
    ...
    if not league_id:
        league_id = HANDBALL_LEAGUES.get(league_name)

    if league_id:
        season = get_current_season_from_api("handball", league_id, league_name, BASE_URL, API_KEY)
        if season:
            LEAGUE_SEASONS[league_id] = season
        else:
            LEAGUE_SEASONS[league_id] = get_best_season("handball")
            print(f"[WARNING] {league_name}: utilisation du fallback")

DEFAULT_SEASON = get_best_season("handball")
    ...
    try:
        ...
        for standing in standings:
            ...
            data.append({
                ...
                "goals_for": points_data.get("for"),
                "goals_against": points_data.get("against"),
...
def main():
    ...
    output_file = OUTPUT_DIR / "handball_data.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2, ensure_ascii=False)

    print(f"[OK] Handball : {payload['total_games']} matchs ({payload['total_teams_games']} equipes + {payload['total_league_games']} ligues) + {payload['total_standings']} classements collectes")
