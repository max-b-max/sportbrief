...
from config.league_ids import VOLLEYBALL_LEAGUES

OUTPUT_DIR = Path("data/raw/api/volleyball")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

CONFIG = get_sport_config("volleyball")
API_KEY = get_api_key("volleyball")
BASE_URL = get_api_url("volleyball")
...
for league in LEAGUES:
    ...
    if not league_id:
        league_id = VOLLEYBALL_LEAGUES.get(league_name)

    if league_id:
        season = get_current_season_from_api("volleyball", league_id, league_name, BASE_URL, API_KEY)
        if season:
            LEAGUE_SEASONS[league_id] = season
        else:
            LEAGUE_SEASONS[league_id] = get_best_season("volleyball")
            print(f"[WARNING] {league_name}: utilisation du fallback")

DEFAULT_SEASON = get_best_season("volleyball")
...
def collect_league_standings(league_id: int, league_name: str, season: int | str) -> list[dict]:
    ...
    try:
        ...
        # Les standings peuvent être dans une liste imbriquée
        ...
        for standing in standings:
            ...
            data.append({
                ...
                "sets_for": points_data.get("for"),
                "sets_against": points_data.get("against"),
...
def main():
    ...
    for team in TEAMS:
        ...
        # Utiliser DEFAULT_SEASON pour les équipes
    ...
    output_file = OUTPUT_DIR / "volleyball_data.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2, ensure_ascii=False)

    print(f"[OK] Volleyball : {payload['total_games']} matchs ({payload['total_teams_games']} equipes + {payload['total_league_games']} ligues) + {payload['total_standings']} classements collectes")
